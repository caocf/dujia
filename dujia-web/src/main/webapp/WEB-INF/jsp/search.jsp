...
<t:head_foot>
  ...
  <title>${s_k}${s_city_name}度假酒店景点门票预订-国美在线</title>
  <meta name="description" content="国美旅行提供${s_k}${s_city_name}度假酒店景点门票预订.有国美,生活美！">
  <meta name="Keywords" content="${s_city_name}度假酒店门票预订,周边游线路">
  ...
  <jsp:attribute name="footer">
    ...
    <script>
      var baseAddress="/search/t${search.request.title}-d${search.request.days}-c${search.request.targetCityId}-s${search.request.scenic}-h${search.request.hotelGrade}.html";
  ...
  //      只有最小价50
      ...
      //      埋码
      s.pageName="度假周边游:搜索结果页:${s_k}";
      s.channel="度假周边游";
      s.prop1="度假周边游:搜索结果页";
      s.prop2="度假周边游:搜索结果页:${s_k}";
      s.prop3="度假周边游:搜索结果页:${s_k}";
      s.prop4="度假周边游:搜索结果页";
      var s_code=s.t();
  ...
  <jsp:body>
    <div class="content">
      ...
      <div class="breadcrumb">
        <h2><a href="${_domain_lvxing}">旅行</a></h2>&gt;<a href="${_domain}">度假</a>&gt; ${s_k}
              ...
              <dl class="">
                ...
                <dd class="search_item">
                  <c:forEach var="titleonly" items="${search.titles}">
                    <a href="/search/t${titleonly.value}-d${search.request.days}-c${search.request.targetCityId}-s${search.request.scenic}-h${search.request.hotelGrade}.html${conditions}">${titleonly.name} (${titleonly.count})</a>
                  </c:forEach>
                </dd>
              </dl>
            </c:if>
              ...
              <dl class="">
                ...
                <dd class="search_item">
                  <c:forEach var="dayonly" items="${search.days}">
                    <a href="/search/t${search.request.title}-d${dayonly.value}-c${search.request.targetCityId}-s${search.request.scenic}-h${search.request.hotelGrade}.html${conditions}">${dayonly.name}</a>
                  </c:forEach>
                </dd>
              </dl>
            </c:if>
              ...
              <dl class="">
                ...
                <dd class="search_item">
                  <c:forEach var="sceniconly" items="${search.scenics}">
                    <a href="/search/t${search.request.title}-d${search.request.days}-c${search.request.targetCityId}-s${sceniconly.value}-h${search.request.hotelGrade}.html${conditions}" value="${sceniconly.value}">${sceniconly.name}</a>
                  </c:forEach>
                </dd>
              </dl>
            </c:if>
              ...
              <dl class="">
                ...
                <dd class="search_item">
                    <c:forEach var="targetonly" items="${search.citys}">
                       <a href="/search/t${search.request.title}-d${search.request.days}-c${targetonly.value}-s${search.request.scenic}-h${search.request.hotelGrade}.html${conditions}" value="">${targetonly.name}</a>
                    </c:forEach>
                </dd>
              </dl>
            </c:if>
              ...
              <dl class="">
                ...
                <dd class="search_item">
                  <c:forEach var="staronly" items="${search.hotelGrades}">
                    <a href="/search/t${search.request.title}-d${search.request.days}-c${search.request.targetCityId}-s${search.request.scenic}-h${staronly.value}.html${conditions}">${staronly.name}</a>
                  </c:forEach>
                </dd>
              </dl>
            </c:if>
          ...
          <div class="selected">
            <p class="count"><i>${search.count}</i> 条符合条件的线路</p>
              <c:if test="${not empty s_theme}">
               <span>${s_theme}<a href="/search/t-d${search.request.days}-c${search.request.targetCityId}-s${search.request.scenic}-h${search.request.hotelGrade}.html"></a></span>
              </c:if>
              <c:if test="${not empty s_days}">
                <span>${s_days}<a href="/search/t${search.request.title}-d-c${search.request.targetCityId}-s${search.request.scenic}-h${search.request.hotelGrade}.html"></a></span>
              </c:if>
              <c:if test="${not empty s_scenery}">
                <span>${s_scenery}<a href="/search/t${search.request.title}-d${search.request.days}-c${search.request.targetCityId}-s-h${search.request.hotelGrade}.html"></a></span>
              </c:if>
              <c:if test="${not empty s_city}">
                <span>${s_city}<a href="/search/t${search.request.title}-d${search.request.days}-c-s${search.request.scenic}-h${search.request.hotelGrade}.html"></a></span>
              </c:if>
              <c:if test="${not empty s_hotelGrade}">
                <span>${s_hotelGrade}<a href="/search/t${search.request.title}-d${search.request.days}-c${search.request.targetCityId}-s${search.request.scenic}-h.html"></a></span>
              </c:if>
              <c:if test="${not empty s_theme || not empty s_days || not empty s_scenery || not empty s_city || not empty s_hotelGrade}">
                <a href="/search/t-d-c-s-h.html">清除</a>
              </c:if>
          </div>
        </div>
      </c:if>
              ...
              <ul class="list_box clearfix">
                <c:forEach items="${search.products}" var="pro">
                  <li title="${pro.subTitle}">
                    ...
                    <div class="pro_price">
                      <p class="price">¥ <i><fmt:formatNumber type="number" value="${pro.productMinPrice}" pattern="0" maxFractionDigits="0"/></i> ${pro.sellUnit}</p>
